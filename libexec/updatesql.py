...
def InsertData(ip,cur,flag):
	...
	if count > 1:
		...
		percent = 1.0
		try:
			cur.execute("insert into %s values(0,'%s','%s','%s',%d,%d,%f,1,'%s')" % (tablename,ip,time,time,find,total,percent,"Open Resolver Discovery"))
		except MySQLdb.Error,e:
	...
	try:
		cur.execute("update %s set last_time='%s',find=%d,total=%d,percent=%f,state=1 where IP='%s'" % (tablename,time,find,total,percent,ip))
	except MySQLdb.Error,e:
	 	print "update %s failed!" % ip
	 	return
	print "update %s success!" % ip
